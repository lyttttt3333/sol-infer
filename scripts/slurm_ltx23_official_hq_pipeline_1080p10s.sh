...
#SBATCH --mem=0
#SBATCH -t 03:00:00
#SBATCH -J ltx23-official-hq
#SBATCH -o outputs/slurm/ltx23-official-hq-%j.out
#SBATCH -e outputs/slurm/ltx23-official-hq-%j.err
...
ROOT="${ROOT:-outputs/ltx23-official-hq-pipeline-1080p10s}"
OUT_DIR="$ROOT/official_hq"
...
.conda/ltx23/bin/python -m ltx_pipelines.ti2vid_two_stages_hq \
  --checkpoint-path "$CHECKPOINT_PATH" \
  --distilled-lora "$DISTILLED_LORA" \
  --distilled-lora-strength-stage-1 0.25 \
  --distilled-lora-strength-stage-2 0.5 \
...
summary = {
    "variant": "official_ti2vid_two_stages_hq",
