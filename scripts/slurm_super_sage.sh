...
#SBATCH --output=/home/yitongl/cosmos3-run/super-sage-%j.out
#SBATCH --error=/home/yitongl/cosmos3-run/super-sage-%j.out
set -uo pipefail
...
# include /home pylibs (sageattention) on the path for the worker subprocess
export PYTHONPATH="$REPO/python:/home/yitongl/.pylibs:${PYTHONPATH:-}"
...
echo "sageattention import check:"; $PYTHON -c "from sageattention import sageattn; print('  sageattn OK')" 2>&1 | grep -iE "sageattn OK|Error|No module" | head -1
...
run(){ local out=$RUN_BASE/super-sage/$1; mkdir -p "$out"
  ...
  stdbuf -oL -eL "$PYTHON" -m sglang.multimodal_gen.runtime.entrypoints.cli.main generate \
    ...
    --output-file-path "$out/out.mp4" --perf-dump-path "$out/perf.json" 2>&1 | grep -E "finished in [0-9]|Error executing|Generation failed|not supported|sage" | tail -3
...
run fa_baseline fa        5680 31080
run sage_int8   sage_attn 5682 31082
echo "[$(date)] super-sage done"
