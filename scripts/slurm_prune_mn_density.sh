...
#SBATCH --output=/home/yitongl/cosmos3-run/prune-mnd-%j.out
#SBATCH --error=/home/yitongl/cosmos3-run/prune-mnd-%j.out
set -uo pipefail
R="${1:?usage: sbatch slurm_prune_mn_density.sh <ratio e.g 0.7>}"
RTAG=$(echo "$R" | tr -d .)              # 0.7 -> 07
PB=$((5800 + (10#$RTAG - 7)*20))         # 07->5800 08->5820 09->5840 (distinct per density)
...
cd "$REPO"; echo "[$(date)] $(hostname)  DENSITY r=$R (RTAG=$RTAG, port base $PB)"
...
# Isolate pruning: FP4/TeaCache/compile OFF. Fixed feat_norm, prev comp. r from arg.
...
export SGLANG_COSMOS3_PRUNE_RATIO=$R SGLANG_COSMOS3_PRUNE_METHOD=feat_norm SGLANG_COSMOS3_PRUNE_COMPENSATION=prev
...
gen(){ # tag prune_steps(or 'none') portoff   [first m / last n full]
  local out=$RUN_BASE/prune-mn-d/r$RTAG/$1; mkdir -p "$out"
  if [ "$2" = "none" ]; then unset SGLANG_COSMOS3_PRUNE_STEPS; else export SGLANG_COSMOS3_PRUNE_STEPS="$2"; fi
  echo "[$(date)] === r$RTAG/$1 (PRUNE_STEPS=$2) ==="
  stdbuf -oL -eL "$PYTHON" -m sglang.multimodal_gen.runtime.entrypoints.cli.main generate \
    ...
    --scheduler-port $((PB+$3)) --master-port $((PB+25000+$3)) \
    --output-file-path "$out/out.mp4" --perf-dump-path "$out/perf.json" 2>&1 | grep -E "token-prune\] active|finished in [0-9]|Error executing" | tail -4
  $PYTHON -c "import json;d=json.load(open('$out/perf.json'));s={x['name']:x['duration_ms'] for x in d['steps']};print('  >>> r$RTAG/$1: denoise %.1fs'%(s['Cosmos3DenoisingStage']/1000))" 2>/dev/null || echo "  >>> r$RTAG/$1: FAILED"
}
gen m0n0 none   0
gen m3n3 "3-31" 2
gen m5n0 "5-34" 4
