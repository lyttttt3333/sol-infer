...
#SBATCH --mem=0
#SBATCH -t 04:00:00
#SBATCH -J ltx23-official-hq-rt
#SBATCH -o outputs/slurm/ltx23-official-hq-rt-%j.out
#SBATCH -e outputs/slurm/ltx23-official-hq-rt-%j.err
...
ROOT="${ROOT:-outputs/ltx23-official-hq-runtime-only-1080p10s}"
OUT_DIR="$ROOT/official_hq_runtime_only"
...
mkdir -p outputs/slurm outputs/.cache/huggingface outputs/.cache/xdg outputs/.cache/torch outputs/.cache/triton outputs/.cache/torchinductor outputs/.cache/torch_extensions outputs/.cache/cuda outputs/.tmp "$OUT_DIR"
...
echo "[run] official HQ runtime-only benchmark -> $OUT_VIDEO"
.conda/ltx23/bin/python scripts/benchmark_ltx23_official_hq_runtime.py \
  --checkpoint-path "$CHECKPOINT_PATH" \
  --distilled-lora "$DISTILLED_LORA" \
  --spatial-upsampler-path "$SPATIAL_UPSAMPLER" \
...
echo "[done] official HQ runtime-only video: $OUT_VIDEO"
echo "[done] official HQ runtime-only summary: $SUMMARY_JSON"
