...
#SBATCH --output=/home/yitongl/cosmos3-run/fp4-e2e-%j.out
#SBATCH --error=/home/yitongl/cosmos3-run/fp4-e2e-%j.out
set -uo pipefail
...
gen(){ # tag fp4 scheduler master  (fp4=0/1)
  local out=$RUN_BASE/fp4-e2e/$1; mkdir -p "$out"
  export SGLANG_COSMOS3_FP4_LINEAR=$2
  echo "[$(date)] === $1 (FP4_LINEAR=$2) ==="
  stdbuf -oL -eL "$PYTHON" -m sglang.multimodal_gen.runtime.entrypoints.cli.main generate \
    ...
    --output-file-path "$out/out.mp4" --perf-dump-path "$out/perf.json" 2>&1 | grep -E "fp4-linear\]|finished in [0-9]|Error executing|Generation failed|Unsupported|assert|RuntimeError" | tail -4
  $PYTHON -c "import json;d=json.load(open('$out/perf.json'));s={x['name']:x['duration_ms'] for x in d['steps']};print('  >>> $1: denoise %.1fs (%.0f ms/step)'%(s['Cosmos3DenoisingStage']/1000,s['Cosmos3DenoisingStage']/35))" 2>/dev/null || echo "  >>> $1: FAILED (no perf.json)"
}
gen baseline 0 5700 31100
gen fp4      1 5702 31102
echo "[$(date)] fp4-e2e done"
