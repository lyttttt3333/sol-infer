...
- Gate-to-output compile mode tuning was tested on the exact retained video self-attention shapes. The current `max-autotune-no-cudagraphs` mode remained fastest: stage-1 video self-attention was `1.097ms` vs `1.123ms` for default, `1.404ms` for `max-autotune`, and `1.412ms` for `reduce-overhead`; stage-2 video self-attention was `1.405ms` vs `1.441ms`, `1.783ms`, and `1.820ms` respectively.
- Gate-to-output plus residual/AdaLN compile was tested as a CODA-style `GEMM + residual + RMSNorm/scale/shift` candidate. It improved over a pure torch residual-norm baseline but was slower than the retained `compiled_gate_to_out + fused AdaLN` path: stage-1 video self-attention was `1.485ms` vs `1.458ms` (`0.982x`), and stage-2 video self-attention was `1.900ms` vs `1.839ms` (`0.968x`).
- Final video output LayerNorm plus scale/shift fusion (`SGLANG_LTX2_FUSED_OUTPUT_LAYER_NORM=1`) was tested as a CODA-style final `LayerNorm + modulation` candidate. The isolated final-output microbenchmark improved video shapes (`3.360x` stage 1, `3.370x` stage 2) but was slower for audio and rejected on the full 1080p/10s stack: `64.408s` full request, `60.327s` through video VAE decode.
- Tiled video VAE decoder compile mode tuning was tested on the current best 1080p/10s stack. The retained `max-autotune-no-cudagraphs` mode remained fastest at `1.496s` video VAE decode and `5.913s` total decode stage; `default` was slower at `1.699s` video VAE decode and `6.124s` total decode stage, `reduce-overhead` was slower at `9.015s` total decode stage, and `max-autotune` was slower in a profiled full run at `4.774s` video VAE decode and `8.257s` total decode stage.
- VAE non-causal temporal padding direct slicing for `pad_count == 1` was tested as a shape-specific LTX2.3 decoder candidate. The isolated temporal-padding microbenchmarks were faster (`1.33x-1.93x`) with exact output match, but the full 1080p/10s stack was slower and the candidate was rejected: `67.873s` full request, `63.552s` through video VAE decode.
...
- Stage-1-video-only current FFN compile (`SGLANG_LTX2_COMPILE_CURRENT_FFN_STAGE1_VIDEO=1`) slightly improved the isolated stage-1 video FFN microbenchmark (`1.014x`) but was rejected on top of the current best full 1080p/10s stack: `66.921s` full request, `62.576s` through video VAE decode. The same microbenchmark was slower for stage-2 video (`0.975x`) and audio (`0.77-0.86x`), so those paths were not enabled for the full candidate.
...
- Standalone gate-logits projection replacement was rejected at microbench scale: the Triton N=32 path was neutral to slower for video (`1.008x` stage 1, `0.993x` stage 2) and only modestly positive for audio (`1.055x` stage 1, `1.114x` stage 2), while torch compile was positive only for stage-1 audio (`1.302x`) and slower for video and stage-2 audio. The retained `SGLANG_LTX2_FUSED_AUDIO_QKVG=1` path already covers the dominant audio self-attention gate projection.
